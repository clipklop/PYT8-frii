...
class DoneCommand(BaseCommand):
    ...
    @staticmethod
    def _load_item_classes():
        from models import DoneItem, UnDoneItem
    ...
    def perform(self, objects, *args, **kwargs):
        ...
        while True:
            try:
                selection = int(input_function('Input item # to finish it: '))
        ...
        selected_class = classes[selected_key]
        print('Finished: {}'.format(str(selected_class.__name__)))
        print()

        new_object = selected_class.construct()

        objects.append(new_object)
        print('Finished {}'.format(str(new_object)))
        print()
        return new_object
